...
`bundle.resources` glob in [`src-tauri/tauri.conf.json`](../tauri.conf.json) and
resolved at runtime from the resource directory (see
[`src-tauri/src/models.rs`](../src/models.rs) → `candidate_paths`).
...
with `"bundled": true` in
[`src-tauri/model-config.default.json`](../model-config.default.json)) are
intended to be populated here as part of a release packaging step.
